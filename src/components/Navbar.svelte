<script>
  ...
  <svelte:window bind:scrollY={y}/>
    ...
    <div class="w-full container mx-auto  flex justify-between px-4">
     ...
     <main class=" flex  items-start px-7 my-auto">
        <div style="font-family: Museo-Sans, sans-serif;" class="flex my-auto  {scrolled?'':''}">
            ...
            <a href="/#FocusAreas" class="md:flex page-scroll hidden cursor-pointer  px-4 py-3  md:py-1 uppercase   text-sm hover:text-green-900">Focus Areas</a>
   ...
   ">
     <div class=" bg-white top-4 absolute spl_cursor">   
        <button on:click="{()=> menu_open = !menu_open }" class="z-50 hidden spl_cursor bg-white cursor-pointer fixed right-5 w-10 h-10 text-xl ml-auto transition duration-150 leading-none px-2 py-1  -solid -transparent rounded bg-transparent block outline-none focus:outline-none" type="button">
